...
## Phase 5b の結果（動力付き rocker_compass, push-off 増強）

円弧足 rocker_compass（Phase 3.5）に pre-emptive **push-off**（後脚軸方向の撃力）を足し、
受動歩行に対して push-off がエネルギーを注入する様子を固定勾配 γ=0.030 で観察した。連続相は
rocker_compass と同一で、heel-strike を「push-off 撃力 → 角運動量保存衝突 → 脚交換」の合成写像に
差し替える。後脚軸の撃力が hip を通るため eq1 のみにモーメントを追加する（実装前 de-risk で検証）。

- **動力リミットサイクル（γ=0.030, push_off=0.08）**: y*=(0.29247, −1.20217, −0.85407)、
  max|λ|=0.462（安定）。不動点近傍からの 20 歩シミュで deviation が単調減衰（3.6e-3 → 1.3e-9）し収束。
- **push-off→0 退化ゲート**: push_off=0 で Phase 3.5 検証済み受動サイクル (0.30844, −1.26256, −0.87914)、
  max|λ|=0.432 に全 stride 一致（push_off 増加で 0.432→0.462）。
- **エネルギー注入**: push-off は衝突後の運動エネルギーを増やす（テストでゲート化）。歩行可能勾配を
  平地側へ拡張することも de-risk の継続で確認（γ=0.030→~0.002）。
- **R=0.3 は平地 γ=0 に届かない**: rocker は push-off を加えても完全な平地に到達する前に fold で
  サイクルが消失する（記録された知見であって平地歩行のデモではない）。これは Phase 4a.1 の
  車輪極限 fold と響き合う。
