"""Phase 3.6 デモ: rocker-foot kneed walker のリミットサイクル発見 → 多歩シミュ → アニメ生成。

Usage: uv run python scripts/walk_rocker_kneed.py [--gamma-deg <published>] [--strides 30] [--perturb 0.005]
"""
...
from crane.viz import animate_rocker_kneed, plot_phase_portrait
...
def main() -> None:
    ...
    run_dir = new_run_dir(f"rocker_kneed_g{args.gamma_deg:g}deg")
    ...
    animate_rocker_kneed(strides, p.l_t, p.l_s, p.R, gamma, run_dir / "walk.mp4")
