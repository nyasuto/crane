...
- 続き **Phase 5c（R 掃引で能動 vs 受動 basin 比較＝本命仮説）** で検証。

## Phase 5c: 能動 vs 受動 basin 比較（本命仮説）— 完了 (2026-06-17)

能動歩行アークの締めくくり。固定勾配 γ=0.030 で R×push_off 格子の能動 basin_fraction(R,P) を測り、
受動 basin（Phase 4a.1）と比較。`basin_slice`（Phase 4a）＋ `powered_rocker_compass`（Phase 5b）の
再利用で新規物理なし。ぽんぽこ殿の仮説「受動で良い個体は能動でも強いか／制御は形態差を消すか」を検証。
...
argmax_R=0.6（全 P）、CV（R間）0.833→0.826→0.820。
...
- **受動の順位は能動でも完全に保存**: basin 最大は常に R≈0.6、順序 0.05<0.2<0.4<0.6 が全 push-off で不変。
  受動で basin の大きい形態は能動下でも最強のまま。**形態が支配的**。
- **制御は形態差をほとんど平坦化しない**: CV 0.833→0.820（~1.5%）。basin(R) 曲線3本がほぼ重なる。
- 一様 push-off はこの系で basin をむしろ僅かに縮小（R=0.6: 0.430→0.410）。同じコントローラを全員に配っても
  頑健性は上がらず、形態の優劣がそのまま残る。
- **結論: 形態（受動 basin）こそ頑健性の支配的決定因子であり、一様な能動制御では形態の優劣を覆せない。**
...
- 今後の課題: R ごとチューニング push-off（同エネルギー/同速度で揃える）、能動制御の種類（hip トルク等）依存、Phase 4b（物理エンジン種戻し）。
