...
- [#19](https://github.com/nyasuto/crane/issues/19) **受動歩行→車輪の連続軸**: rocker-foot 半径 R を
  脚長に向けて掃引し、各 R で basin_fraction と衝突エネルギー損失（効率）を測る。「コンパスは車輪への
  漸近、円弧足はそれを車輪に近づける（最も車輪的＝最大 basin）。膝は逆に不整地適応と引き換えに
  basin/効率を犠牲にする」という概念枠組みの検証。rimless wheel = 最簡受動歩行モデルの provenance を
  McGeer 1990a で裏取りしてから主張に使う（PR #18 マージ後に着手）
