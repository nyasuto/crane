...
# 本規約断面 y = (θ_st, θ̇_st, θ̇_th, θ̇_sh):
#   θ_st⁺ = q1 + γ = 0.1877 + 0.0504 = 0.2381
#   （検算: θ_th⁺ = q2 + γ = −0.2380 = −θ_st⁺、断面の位置拘束 θ_th = θ_sh = −θ_st
#    が 4 桁丸め内で成立 ✓）
#   速度はオフセット変換で不変: θ̇_st⁺ = −1.1014, θ̇_th⁺ = θ̇_sh⁺ = −0.0399
# 注意: 原典の heel-strike は q̇3⁺ = q̇2⁺ を仮定するため脛速度は大腿と同値。
# 本実装（脛の保存則 3 本目）では不動点の θ̇_sh⁺ が θ̇_th⁺ から僅かにずれうる。
# あくまで初期推定として使う。
SECTION_GUESS: tuple[float, float, float, float] = (0.2381, -1.1014, -0.0399, -0.0399)
...
# (b) 断面が 3D 縮約（q̇3 = q̇2 拘束込み）で本実装の 4D 断面と次元が違う。
