...
定式化の照合（cross-check 済み 2026-06-13）:
    ...
    heel-strike（Sec. 3.2.2, eq. 3.5）: 保存則 2 本 —
        ...
        **運動学的に仮定**（衝突を通して新 swing 脚を剛体扱いし、直後に
        大腿速度のままアンロック = rigid-through-collision）。
        本プロジェクトも同一定式化を採用（plan revision 7094d2a）。kneed.py の
        heelstrike_map / _build() は eq 3.5 と一致する: 保存則 (i)(ii) を 2×2 で
        解き、θ̇_sh⁺ = θ̇_th⁺ を構造的に設定する。3 本独立の保存則（脛を knee
        回りで保存し θ̇_sh⁺ を別に解く）は当初案だったが不採用。
        したがって原典との定式化の相違はない（cross-check で確認）。
...
# 照合許容（相対）: 不動点は 4 桁印刷だが (a) 原典は dt=1e-3 の固定刻み前進積分
# で得た値（イベント検出は 1e-20 精度だが固定刻み積分のバイアスが残る。本実装は
# solve_ivp の適応高精度 RK なので、この差は積分器の違いとして残る）、
# (b) STEP_PERIOD は図読み取り ±2%。以上を見込んで ±5% とする
# （heel-strike 定式化は原典 eq 3.5 と一致するので系統差はない。docstring 参照）
